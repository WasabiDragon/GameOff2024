		...
		foreach(Paper paper in _paperStacks)
		{
			...
			if(info.important)
			{
				...
				if(info.attachedPapers != null && info.attachedPapers.Count > 0)
				{
					...
					{
						if(solved || sheet.GetComponent<Paper>().GetInfo().displayText == info.originalText || sheet.GetComponent<Paper>().GetInfo().displayText == info.smallTextOriginal)
						{
				...
				_result.Add("Paper Number #"+paper.gameObject.name.Substring(0,4)+": "+paperOutcome+".");
			}
		}
		return _result;
	}
